...
function encryptTestGitHubToken(token) {
  const key = crypto
    .createHash('sha256')
...
test('listBuildRuns rejects an invalid repository id as not found', async () => {
  ...
  Repo.findOne = async () =>
    assert.fail('invalid ids should not query MongoDB');
...
test('analyzeBuildRun rejects access to another user repository', async () => {
  ...
  User.findById = () => ({
    select: async () => ({
      ...
      githubAccessTokenEncrypted: encryptTestGitHubToken(
        'github-test-token'
      ),
    }),
  });
...
test('analyzeBuildRun stores mocked GitHub/Gemini analysis for an owned failed run', async () => {
        ...
        candidates: [
          {
            content: {
              parts: [
                {
                  text: 'Check the failed test step.',
                },
              ],
            },
          },
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(
    response.body.aiAnalysis,
    'Check the failed test step.'
  );
...
test('startGithubLink returns a short-lived single-purpose link URL', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.match(
    response.body.linkUrl,
    /^\/api\/auth\/github\/link\?token=/
  );
...
test('githubLink rejects invalid and mis-purposed link tokens', async () => {
  ...
  await githubAuthController.githubLink(
    {
      query: {
        token: 'not-a-token',
      },
...
test('github link state cookie binding rejects tampered user ids and signatures', () => {
  ...
  const cookie = githubAuthController.encodeLinkState(
    'state-abc',
    userId
  );
  ...
  assert.equal(
    githubAuthController.parseLinkState(tamperedUser),
    null
  );
  ...
  assert.equal(
    githubAuthController.parseLinkState(badSignature),
    null
  );
});
...
test('githubLinkCallback attaches GitHub to the existing local user only', async () => {
  ...
  User.create = async () =>
    assert.fail('linking must not create a new User');

  axios.post = async () => ({
    status: 200,
    data: {
      access_token: 'github-live-token',
    },
  });
  ...
  await githubAuthController.githubLinkCallback(
    {
      ...
      headers: {
        cookie: `github_oauth_state=${cookie}`,
      },
    },
    response
  );
  ...
  assert.equal(
    redirectUrl.searchParams.get('github'),
    'linked'
  );

  assert.equal(
    redirectUrl.searchParams.get('username'),
    'octocat'
  );
  ...
  assert.match(
    savedUser.githubAccessTokenEncrypted,
    /^[^.]+[.][^.]+[.][^.]+$/
  );
...
test('githubLinkCallback rejects a GitHub account bound to another user', async () => {
  ...
  const savedUser = {
    ...
    githubId: null,
    save: async () =>
      assert.fail('conflict must not save'),
  };

  User.findById = async () => savedUser;

  User.findOne = async (filter) =>
    filter.githubId === '12345'
      ? {
...
test('githubLinkCallback rejects silently replacing an already-linked GitHub account', async () => {
  ...
  assert.match(
    redirectUrl.searchParams.get('message'),
    /Another GitHub account is already connected/
  );
...
test('githubLinkCallback rejects a tampered linking cookie before any user lookup', async () => {
  ...
  User.findById = async () =>
    assert.fail('tampered cookie must not load a user');

  axios.post = async () =>
    assert.fail('tampered cookie must not exchange a code');
  ...
  await githubAuthController.githubLinkCallback(
    {
      ...
      headers: {
        cookie: `github_oauth_state=${tampered}`,
      },
    },
    response
  );
